<?php
/*
  $Id$

  CE Phoenix, E-Commerce made Easy
  https://phoenixcart.org

  Copyright (c) 2021 Phoenix Cart

  Released under the GNU General Public License
*/

const TABLE_HEADING_MODULES = 'Module';
const TABLE_HEADING_SORT_ORDER = 'Sortierung';
const TABLE_HEADING_ACTION = 'Aktion';
const TABLE_HEADING_ENABLED = 'Aktiviert';
const TABLE_HEADING_GROUP = 'Gruppe';
const TABLE_HEADING_WIDTH = 'Container';
const TABLE_HEADING_DISPLAY_GROUP = 'Display';

const TEXT_INFO_VERSION = 'Version:';
const TEXT_INFO_ONLINE_STATUS = 'Online-Status';
const TEXT_INFO_API_VERSION = 'API Version:';

const TEXT_MODULE_DIRECTORY = 'Modul Verzeichnis:';

const ERROR_MODULE_UNMET_REQUIREMENT = 'Dieses Modul hat eine unerfüllte Abhängigkeit.';
const ERROR_MODULE_HAS_DEPENDENTS = 'Andere Module sind von diesem Modul abhängig.';

const WARNING_INVALID_USE_FUNCTION = 'Die Verwendungsfunktion (%s) für "%s" ist ungültig.';
